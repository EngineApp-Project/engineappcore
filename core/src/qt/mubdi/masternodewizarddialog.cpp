    ...
    clientModel(_clientModel)
{
    ...
    ui->labelMessage1a->setText(formatHtmlContent(
                formatParagraph(tr("To create a MuBdI Masternode you must dedicate %1 (the unit of EAPP) "
...
bool MasterNodeWizardDialog::createMN()
{
    ...
    6) use those values on the mubdi-mn.conf
     */
    ...
    std::string strConfFile = "mubdi-mn.conf";
    std::string strDataDir = GetDataDir().string();
    fs::path conf_file_path(strConfFile);
    if (strConfFile != conf_file_path.filename().string()) {
        throw std::runtime_error(strprintf(_("mubdi-mn.conf %s resides outside data directory %s"), strConfFile, strDataDir));
    }

    fs::path pathBootstrap = GetDataDir() / strConfFile;
    if (!fs::exists(pathBootstrap)) {
        returnStr = tr("mubdi-mn.conf file doesn't exists");
        return false;
    }
    ...
    if (!streamConfig.good()) {
        returnStr = tr("Invalid mubdi-mn.conf file");
        return false;
    }
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
            ...
            if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                streamConfig.close();
                returnStr = tr("Error parsing mubdi-mn.conf file");
                return false;
            }
        }
        lineCopy += line + "\n";
    ...
    if (lineCopy.size() == 0) {
        lineCopy = "# Masternode config file\n"
                   ...
                   "# Example: mn1 127.0.0.2:port_p2p 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0"
                   "#";
    }
    lineCopy += "\n";
    ...
    fs::path pathNewConfFile = AbsPathForConfigVal(fs::path("mubdi-mn.conf"));
