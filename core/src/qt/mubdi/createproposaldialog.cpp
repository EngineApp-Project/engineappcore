...
#include "qt/mubdi/createproposaldialog.h"
#include "qt/mubdi/forms/ui_createproposaldialog.h"
...
CreateProposalDialog::CreateProposalDialog(MuBdIGUI* parent, GovernanceModel* _govModel, WalletModel* _walletModel) :
    QDialog(parent),
...
void CreateProposalDialog::setupPageOne()
{
    ...
    setEditBoxStyle(ui->labelURL, ui->lineEditURL, "e.g https://forum.mubdi/proposals/best_proposal_ever");
...
void CreateProposalDialog::setupPageTwo()
{
    ...
    setEditBoxStyle(ui->labelAmount, ui->lineEditAmount, "e.g 500 EAPP");
...
bool CreateProposalDialog::propaddressChanged(const QString& str)
{
    if (!str.isEmpty()) {
        ...
        const bool valid = walletModel->validateAddress(trimmedStr, false) && !isShielded;
