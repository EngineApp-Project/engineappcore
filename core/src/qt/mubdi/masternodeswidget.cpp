    ...
    if (dialog->exportMN) {
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the engineapp.conf file\nof your remote server and start it, "
...
void MasterNodesWidget::onDeleteMNClicked()
{
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (!streamConfig.good()) {
            inform(tr("Invalid mubdi-mn.conf file"));
            return;
        }
        ...
        for (std::string line; std::getline(streamConfig, line); linenumber++) {
            ...
            if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                ...
                if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                    streamConfig.close();
                    inform(tr("Error parsing mubdi-mn.conf file"));
                    return;
                }
            }
        ...
        inform(tr("mubdi-mn.conf file doesn't exists"));
    }
}
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= mnCollateralAmount) {
        inform(tr("Not enough balance to create a masternode, %1 required.")
            .arg(GUIUtil::formatBalance(mnCollateralAmount, MuBdIUnits::MuBdI)));
        return;
    }
    showHideOp(true);
