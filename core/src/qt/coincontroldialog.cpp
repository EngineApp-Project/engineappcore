...
TotalAmounts CoinControlDialog::getTotals() const
{
    ...
    if (t.nQuantity > 0) {
        ...
        t.nPayFee = GetMinRelayFee(t.nBytes, false);
...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select engineapp Outputs to Spend" :
            "Select EAPP to Spend");
    ...
    ui->labelCoinControlAmount->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, t.nAmount));
    ui->labelCoinControlFee->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, t.nPayFee));
    ui->labelCoinControlAfterFee->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, t.nAfterFee));
    ...
    ui->labelCoinControlChange->setText(MuBdIUnits::formatWithUnit(nDisplayUnit, t.nChange));
    ...
    // tool tips
    QString toolTip3 = tr("This label turns red, if recipient receives an amount smaller than %1 (transparent)."
            ).arg(MuBdIUnits::formatWithUnit(nDisplayUnit, GetDustThreshold(dustRelayFee)));
                                          ...
                                          const bool isChange)
{
    ...
    // amount
    itemOutput->setText(COLUMN_AMOUNT, MuBdIUnits::format(nDisplayUnit, nValue));
    itemOutput->setToolTip(COLUMN_AMOUNT, MuBdIUnits::format(nDisplayUnit, nValue));
...
{
    payAmounts.emplace_back(amount);
}
