...
QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZMuBdI)
{
    return (amount == 0) ? ("0.00 " + MuBdIUnits::name(nDisplayUnit, isZMuBdI)) : MuBdIUnits::floorWithUnit(nDisplayUnit, amount, false, MuBdIUnits::separatorAlways, true, isZMuBdI);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter engineapp address (e.g. %1)").arg("mubFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", MUBDI_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MuBdI (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MuBdI (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "MuBdI.lnk";
}
...
{
    return GetAutostartDir() / "MuBdI.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=MuBdI (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=MuBdI (regtest)\n";
        else
            optionFile << "Name=MuBdI\n";
