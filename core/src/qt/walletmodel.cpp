...
{
    return !sporkManager.IsSporkActive(SPORK_9_COLDSTAKING_MAINTENANCE);
}

bool WalletModel::isV6Enforced() const
{
    return Params().GetConsensus().NetworkUpgradeActive(cachedNumBlocks, Consensus::UPGRADE_V6);
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction* transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
    ...
    for (const SendCoinsRecipient& rcp : recipients) {
        { // User-entered mubdi address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
    ...
    for (const SendCoinsRecipient& rcp : transaction.getRecipients()) {
        ...
        {
            bool isStaking = false;
            auto address = Standard::DecodeDestination(rcp.address.toStdString(), isStaking);
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (isStaking)
        return error("Invalid engineapp address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CTxDestination& address, CKeyID& keyID)
{
    if (!IsValidDestination(address))
        return error("Invalid engineapp address");

    const CKeyID* inKeyID = boost::get<CKeyID>(&address);
    if (!inKeyID)
        return error("Unable to get KeyID from engineapp address");
...
// returns a COutPoint of 5000 MUB if found
