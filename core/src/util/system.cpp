...
const char * const MUBDI_CONF_FILENAME = "engineapp.conf";
const char * const MUBDI_PID_FILENAME = "engineapp.pid";
const char * const MUBDI_MASTERNODE_CONF_FILENAME = "engineapp-mn.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "eappd -foo=bar
...
#else
    const char* pszModule = "engineapp";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EAPP";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", MUBDI_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", MUBDI_PID_FILENAME));
